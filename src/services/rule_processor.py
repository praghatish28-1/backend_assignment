...
class RuleProcessor:
    ...
    def evaluate_rule(self, rule, email):
        ...
        if not conditions:
            # Handle empty conditions explicitly
            return match_all if match_all else False
        ...
        for condition in conditions:
            ...
            if field_name.lower() == "date received":
                try:
                    strp_time = time.strptime(value, "%Y-%m-%d")
                    epoch_seconds = time.mktime(strp_time)
                    value = int(epoch_seconds * 1000)
                except ValueError as e:
                    logger.error(f"Invalid date format in condition: {value}")
                    return False
            ...
            current_match = predicate_strategy(email_value, value)

            if match_all:
                current_match = old_match and current_match

            if not current_match and match_all:
                return False
            if current_match and not match_all:
                return True
